...
SANDBOX_HTTP_BODY_LIMIT="${SANDBOX_HTTP_BODY_LIMIT:-256mb}"
SANDBOX_STREAM_REQUEST_LIMIT="${SANDBOX_STREAM_REQUEST_LIMIT:-1mb}"
SANDBOX_STREAM_INPUT_CHUNK_LIMIT="${SANDBOX_STREAM_INPUT_CHUNK_LIMIT:-64kb}"
SANDBOX_STREAM_INPUT_TOTAL_LIMIT="${SANDBOX_STREAM_INPUT_TOTAL_LIMIT:-16mb}"
DEPLOY=0
...
if [ "$DEPLOY" = "1" ]; then
  ...
  docker run -d --restart unless-stopped --privileged \
    ...
    -e SANDBOX_HTTP_BODY_LIMIT="$SANDBOX_HTTP_BODY_LIMIT" \
    -e SANDBOX_STREAM_REQUEST_LIMIT="$SANDBOX_STREAM_REQUEST_LIMIT" \
    -e SANDBOX_STREAM_INPUT_CHUNK_LIMIT="$SANDBOX_STREAM_INPUT_CHUNK_LIMIT" \
    -e SANDBOX_STREAM_INPUT_TOTAL_LIMIT="$SANDBOX_STREAM_INPUT_TOTAL_LIMIT" \
    -p "${HOST}:${PORT}:5050" \
