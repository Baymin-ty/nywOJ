const {
  ...
  updateData,
  clearCase,
  notifySubmissionProgress } = require('./judge');
...
const judgeCode = async (sid, isRejudge) => {
  ...
  try {
    ...
    // compile
    let compileResult;
    await logEvent(sid, 'compile.start', { args: ['g++-9', '-O2', '-std=c++14', '-DONLINE_JUDGE', 'main.cpp', '-o', 'main'] });
    try {
      ...
      await logEvent(sid, 'compile.result', summarizeSandboxResult(compileResult));
    } catch (err) {
    ...
    if (pinfo.type === 1) {
      ...
      // compile spj
      let SPJcompileResult;
      await logEvent(sid, 'spj.compile.start', { args: ['g++-9', '-O2', '-std=c++14', '-DONLINE_JUDGE', 'spj.cpp', '-o', 'spj'] });
      try {
        const res = await axios.post('http://localhost:5050/run', {
          "cmd": [{
        ...
        await logEvent(sid, 'spj.compile.result', summarizeSandboxResult(SPJcompileResult));
      } catch (err) {
    ...
    for (let i in cases) {
      ...
      try {
        ...
        await logEvent(sid, 'case.error', { caseId: cases[i].index, error: summarizeAxiosError(err) });
        throw err;
      }
      ...
      if (runResult.status !== 'Accepted') {
        ...
        usrOutput = (runResult.files && runResult.files.stdout) || '';
        let compareRes = '';
        ...
        if (pinfo.type === 0) {
          ...
          try {
            ...
            await logEvent(sid, 'spj.run.error', { caseId: cases[i].index, error: summarizeAxiosError(err) });
            throw err;
          }
        }
        await logEvent(sid, 'case.compare', {
          caseId: cases[i].index,
          result: compareRes.substring(0, 2) === 'ok' ? 'ok' : 'wa',
