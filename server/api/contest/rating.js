...
// 组队场/作业强制不产生 rating：rating 是个人维度，组队榜单行的 user.uid 是
// teamId 绝不能流入 contestRating；作业无竞技性。所有结算/重建/预览路径都经过本函数。
const ratingIneligible = async (cid) => {
  const contest = await getContest(cid);
  if (!contest) return false;
  if (contest.format === 'homework') return true;
...
const calculateContestRatingChanges = async (cid, baseRatings = null, options = {}) => {
  if (await ratingIneligible(cid)) return [];
