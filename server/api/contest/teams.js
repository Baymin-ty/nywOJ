...
const { loadView, canManageContest, contestStatus } = require('./policy');
...
exports.adminCreateTeam = handler(async (req, res) => {
  ...
  const { cid } = req.body;
  const name = String(req.body.name || '').trim();
  const memberNames = Array.isArray(req.body.members) ? req.body.members : [];
  const allowed = await canManageContest(req, cid);
  if (allowed === null) return fail(res, '无此比赛');
  if (!allowed) return res.status(403).end('403 Forbidden');
  if (!name || name.length > 40) return fail(res, '队名需为 1-40 字符');
  ...
  for (const n of memberNames) {
    const u = await db.one('SELECT uid FROM userInfo WHERE name=? LIMIT 1', [String(n).trim()]);
    if (!u) return fail(res, `无此用户：${n}`);
    const busy = await db.exists('SELECT 1 FROM contestPlayer WHERE cid=? AND uid=?', [cid, u.uid]);
    if (busy) return fail(res, `用户 ${n} 已在本场比赛中`);
    users.push(u.uid);
  }
  if (new Set(users).size !== users.length) return fail(res, '成员重复');
  ...
  const teamId = await db.tx(async (tx) => {
    ...
    for (let i = 0; i < users.length; i++) {
      await tx.query('INSERT INTO contestTeamMember(teamId,uid,isCaptain) VALUES (?,?,?)', [tid, users[i], i === 0 ? 1 : 0]);
      await tx.query('INSERT INTO contestPlayer(cid,uid,teamId) VALUES (?,?,?)', [cid, users[i], tid]);
    }
    return tid;
  });
  return ok(res, { teamId, inviteCode });
